...
class Reader(object):
    ...
    def __init__(self, gpu=True, model_storage_directory=None,
        ...
        if recog_network in ['standard'] + [model for model in recognition_models['gen1']] + [model for model in recognition_models['gen2']]:
            recog_network, model, lang_list = self.selectModel(recog_network)
            ...
            if recognizer:
                self.downloadRecognitionModel(verbose, model, model_path)
            self.setLanguageList(lang_list, model)

        else: # user-defined model
            model_path, recog_config = self.userDefinedModel(recog_network, lang_list)
    ...
    def userDefinedModel(self, recog_network, lang_list):
        ...
        return model_path,recog_config

    def selectModel(self, recog_network):
        ...
        if recog_network in [model for model in recognition_models['gen2']]:
        ...
        return recog_network,model,lang_list

    def downloadRecognitionModel(self, verbose, model, model_path):
